 ...
 *       = 进度上限 16 条
 *
 * **保留就是靠进度的上限实现的**:进度撞到上限就再也发不出去,于是剩下的 3 条
 * 谁也抢不走。`fallback`(人格不可达时信使自己回的那句)与正文互斥,共用同一份保留额。
 *
 * 保留额是 3 而不是 2,因为 `announce` 也占一份 —— 部署结果是最不能丢的一条,
 * 用户接下来说的话都建立在"改动已生效"这个前提上。
 *
 * ## 额度花光了不是绝路
 *
 * 用户随便发一句话就带来新的 `context_token`,计数跟着归零。`/nop` 就是为此存在的
 * 那句"随便的话":什么也不做,只把额度续上。进度的最后一条会附一句提示告诉他这件事
 * (`PROGRESS_CAP_NOTICE`),否则他只看得到进度毫无征兆地停住。
 *
 * ## 为什么按"尝试"计数而不是"成功"
 *
