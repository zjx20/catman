...
**agent 任务没有 \`script\` 那个 \`/work\`。** 它的文件一律写
\`$CATMAN_DATA_DIR/cron_data/<任务名>/\` —— 台账、游标、上期产出、提示词、日志、
随手写的临时文件,都放这儿。每次启动都会确保它存在、权限 0777。

**别往 \`tmp/\` 里放,连临时文件也别。** 那边是会话里脱钩长任务的地盘,随时可以
当垃圾清;而定时任务无人值守,被误清了没人当场发现 —— 它照跑,只是把上次报过的
又报一遍。按归属圈在一起,清理时才只需要判断"这个任务还在不在"。

提示词长了也写进那个目录(\`prompt.txt\`),\`prompt\` 字段只留一句"去读那个文件,
严格照做"。\`task.prompt\` 有 **4000 字符上限**,而且指令留在文件里改起来只是编辑
文件,不用每次 PATCH 任务。
...
改了 \`scripts/evolve/\` 就是 Tier 3:部署上线**不会**让它生效,跑的仍然是 bless 固化到
\`data/deploy/bin/\` 的那份旧副本。要生效得有人重新 bless。

**默认不自己 bless。** 制备完照常汇报「这次是 Tier 3,还欠一次 bless」,什么时候固化
由管理员定 —— 固化会把 \`scripts/evolve/\` 的当前内容变成"以后真正执行的那份",
包括那道判卷子的门本身,而你正是被它判的人。

他明确授权之后可以代跑,两条不能松:

- **固化源必须是他批准过的那棵树。** 跑之前确认工作区干净,且 \`scripts/evolve/\` 与
  \`releases/current\` 里那份逐字节一致(\`cmp\` 一遍)。从一棵没人看过的工作树固化,
  等于把「他批准了哪棵树」和「固化了哪棵树」拆开。
- **\`CATMAN_PIN\` 显式传当前 pinned**,除非他说要挪。\`pinned\` 是信使与守护人格跑的
  那份,"出事时还活着的是哪份代码"不该被顺手改掉。

命令照 README 的「重新 bless」那节抄,别自己临时拼 \`docker run\` —— 少一个 \`-e\` 就是一次
静默的错误固化。管理员机器上可能包了个壳,他指哪个就用哪个。

跑完验四样:四个脚本 cmp 得上、\`data/deploy/env\` 一个字没变、\`pinned\` 是他要的那个、
属主回到 10002。
...
- **Tier 3**:\`scripts/evolve/\`(要重新 bless,规矩见上面那节)、\`docker/\`(要重建镜像)、
