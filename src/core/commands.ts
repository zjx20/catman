...
export const COMMAND_TABLE: readonly CommandDef[] = [
  ...
  },
  {
    ...
    canonical: "/nop",
    aliases: ["/noop"],
    desc: "什么也不做、也不回话 —— 但这条消息本身会把微信那边的发送额度续上,进度停住时发它(不花额度)",
    // 续额靠的不是这条指令做了什么,而是**这条消息存在**:iLink 的每条来信都带来
    // 新的 context_token,信使那边的计数随之归零(见 courier/reply-store.ts)。
    // 所以它的实现只剩一件事:重置进度节流器。幂等,符合 immediate 的约束 ——
    // 而 immediate 正是它需要的:额度耗尽最常发生在长回合跑到一半,
    // 那时队列里的消息还轮不到。
    //
    // 它**不回话**,一个字都不回:确认话术要从它刚续回来的额度里花掉一条,
    // 挤掉的正是用户发它想看的那条进度。理由详见 gateway.ts 的 `case "nop"`。
    //
  ...
  },
  {
    ...
    aliases: ["/publish", "/發布"],
    desc:
      "把制备好的版本部署上线,参数是版本号前 6 位(制备完我会报给你);" +
      "后面可以再跟一个秒数缩短观察期,如 `/发布 abc1234 60`",
