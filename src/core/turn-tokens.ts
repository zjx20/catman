...
export interface TurnContext {
  ...
  feed?: TurnFeed;
  /**
   * 把进度节流器重新开闸(条数与间隔阶梯都从头来)。
   *
   * 由网关在回合跑起来时挂上,节流器本身是那一轮的局部状态 —— 挂出来只为了
   * `/nop`:那条指令走的是 immediate 路径,不经过追加输入,而额度确实在它抵达的
   * 那一刻就续上了。不重开闸的话节流器仍记着"余量为 0"、从此一条不发,
   * 用户看到的就是"发了 /nop 也没用",而那句提示恰恰是我们让他发的。
   */
  resetProgress?: () => void;
  /**
   * 本回合彻底结束(revoke)时兑现。
