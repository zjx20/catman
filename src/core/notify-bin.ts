 ...
 * 只是安静地不工作:
 *
 * - 容器里配了代理,而 `NO_PROXY` 是**整体替换**的:谁在 `.env` 里自定义一次、
 *   忘了把 `127.0.0.1` 抄回去,打自己就会收到一个 503(那是代理说的,不是服务)。
 *   所以这里 `--noproxy '*'` 钉死,不指望配置里记得留那一条;
...
post() {
  # --noproxy '*':打的是本机那个 catman,按定义永远不该走代理。NO_PROXY 是整体
  # 替换的,指望它一直留着 127.0.0.1 不保险 —— 漏了的症状是 503(代理说的,像服务坏了)。
  local payload
