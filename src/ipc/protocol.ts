...
/**
 * 出站消息的类别。它决定的是**发不出去时怎么排队**(courier/outbox.ts 的策略表),
 * 不决定预算 —— 预算对所有类别一视同仁,唯一的例外是 `budget`,
 * 见 courier/reply-store.ts。
 */
export const SEND_KINDS = [
  ...
  /** 模型的答复正文。发不出去排队,一条不丢。 */
  "body",
  /** 提醒类:会话空闲提醒、token 到期告警、定时任务开跑通知。发不出去只留最新一条。 */
  "reminder",
  /** 人格不可达时由信使自己回的那句话。发不出去排队,一条不丢。 */
  "fallback",
  /** 部署结果播报、定时任务产出。发不出去排队,一条不丢。 */
  "announce",
  /**
   * 额度提示:「回信额度已用完,发 /nop 补充额度」。**信使内部专用**,人格发来的
   * 一律被拒(courier/core.ts)。它是预算里唯一能用保留格的类别,见 reply-store.ts。
   *
   * 加一种 kind 而不复用 `reminder`:保留格必须认得出"这条就是那句提示",而认的
   * 唯一依据就是 kind。兼容上没有顾虑 —— 信使跑 pinned、版本比人格老,但这个 kind
   * 只在信使内部产生、内部消费,从不过 IPC;认得它的信使一定也带着用它的代码。
   */
  "budget",
] as const;
...
export interface SendResult {
  ...
  /**
   * 这条来信的 `context_token` 还能再发几条普通消息(已扣掉留给额度提示的那一格)。
   *
   * 字段名是历史遗留:从前进度有单独的额度,人格的节流器据此收缩;现在进度与别的
   * 类别同池,人格也不再看这个数。名字不改 —— 信使跑 pinned,两侧版本不同步,
   * 改名等于让一侧读不懂另一侧。
   */
