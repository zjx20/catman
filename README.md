...
| 「进度就报到这儿」 | 预留 1 |
| 「后面还有没发出去的」 | 预留 1 —— **这一格是硬留的**,见下面那条规矩 |
| 会话空闲提醒 / 部署进展播报 | 不预留 —— 发不出去就进发件队列,见下节 |

**两句交代各占一格,不能合并。** 前一句许诺"接下来直接等答案",而答案分段超过一段时
它就是假话 —— 那时你需要的是后一句。合用一格的话,先说出口的那句会把后一句挤掉,
而被挤掉的恰恰是更要紧的那句。代价是进度从 7 条降到 6 条(阶梯跑满约 4 分钟)。
...
- **排空留余地**:停在还剩 `LIVE_TURN_SENDS` 条时(回执 + 答案 + 那两句交代),而不是把新
  额度榨干 —— 你刚发的那句话也需要它们。停下时队列还没空的话会说一句
  「还有 N 条没发出去,发 `/nop` 我接着发」,**每份 `context_token` 只说一次**。
  这里报得出**准确条数**(排空时队列是稳定的);回合还在跑时那句不报数,
  因为后面还有几段正文要交进来,当场数出的"还有 1 条"下一秒就成了假话。
  两句都**只数正文那类**,不数排队的过期进度 —— 补发一句十分钟前的「🔧 正在跑测试」
  没有价值,而那一格是交代唯一的落脚点。
- **落盘**。信使重启(bless、改配置)时积压里可能正躺着一条已经跑完却没送出去的答案。

那几句交代都由**信使**发,每句每份 `context_token` 只说一次 —— 说这话本身也要花一格。
进度那句的触发条件是**一条进度被拒**,而不是"余量剩 1":这样还带着自己那份余量判断的
旧版助手根本撞不到它(它在上限之前就收手了),新旧两侧不会各说一遍。

**交代永远排在内容前面。** 只要这份 token 的交代还没说出去,内容就不许动最后一格 ——
正文分段发到额度见底时,那一格拿去说「后面还有没发出去的(答案可能只发了一半)」,
而不是再多发半段正文。少半段看得出来,静默看不出来:队列保证了消息不丢,但**没保证
你知道有东西没发出去**,而你看到的只是话说到一半就停了,与卡死无从分辨。这条规矩就是
补那个洞的 —— 真机上因此静默过 14 分钟和 2 小时 24 分,两次都是等用户自己开口才补发。
