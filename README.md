...
**现在 `SEND_BUDGET` 设的是 20,那是在试,不是实测值。** 6 条进度在长回合里确实太紧,
而上面那次观测已经有些日子了,服务端可能早就不是那个行为。代价不对称,所以要盯着:
若 10 仍是硬上限,第 11 条起全败,而正文排在进度后面 —— 用户看到的就是"进度停住、
答案永远不来"。判据是日志里那行 `send #11(前 10 条成功) … 失败 ret=-2`,
真出现就把 `src/courier/reply-store.ts` 里的 `SEND_BUDGET` 改回 10。
...
|---|---|
| 收到回执 | 1 |
| 进度推送 | 最多 17(`MAX_PROGRESS_PER_TOKEN`) |
...
- **排空留余地**:停在还剩 4 条时,而不是把新额度榨干 —— 你刚发的那句话也需要回执、
  答案。停下时队列还没空的话会说一句「还有 N 条没发出去,发 `/nop` 我接着发」,
