...
- **一个 `context_token` 的发送次数是有预算的**。真机实测过的上限是 **10**:第 11 条起
  ...
  预算在 `courier/reply-store.ts` 顶部显式列支:`SEND_BUDGET(20) − 回执 1 −
  RESERVED_SENDS(3)` = `MAX_PROGRESS_PER_TOKEN`。**预留的三条是正文、会话空闲提醒、
  部署结果播报** —— 中间那条容易漏:提醒的前提就是用户没再发消息,而 `replyCtx` 只在
  收到新消息时更新,所以它用的还是上一个回合那份额度,进度吃光了它就永远发不出去。
  **保留就是靠进度的上限实现的**(进度撞上限就再也发不出去,剩下 3 条谁也抢不走),
  所以改进度上限等于改保留额,改之前先把这笔账重算一遍。
  ⚠️ **`SEND_BUDGET` 现在是 20,那是在试,不是实测值。** 若 10 仍是真上限,第 11 条起
  全败 —— 而正文排在进度后面,症状就是"进度停住、答案永远不来"。判据是日志里
  `send #11(前 10 条成功) … 失败 ret=-2 prepare failed`,真出现就改回 10。
- **额度花光了不是绝路,`/nop` 就是那条出路**:用户随便发一句话都带来新的
  `context_token`,信使那边的计数随之归零。所以最后一条进度会附上这个口令
  (`PROGRESS_CAP_NOTICE`),而口令这一侧要成立需要**两件事都做到**:网关收到 `/nop` 时
...
- **发送预算的账只有一本,在信使**(`courier/reply-store.ts`)。网关**不记**,进度的总量
  上限每次现问渠道(`Channel.progressBudget`)。曾经两处各记一份,而且已经对不上
  (当时网关 `10−1−2=7`,信使 `10−1−3=6`)—— 后果不是超发(信使会拒),是**每个长回合都多发
  一条注定被拒的进度**,并且「进度就报到这儿」那句提示**永远不会触发**:信使在第 6 条
  就拒了,网关要到第 7 条才提示,于是用户看到进度毫无征兆地停住,正是那句提示要防的事。
  两处常量碰巧相等不叫单一权威,叫还没出事。渠道说不上来余量(stdin / dashboard 没有
  发送预算这个概念)就**不设总量上限** —— 给它们套一个凭空来的上限,等于把 iLink 的
  协议限制强加到根本没有这个限制的渠道上。bridge **只信成功那次报的余量**:
  「信封读不懂」「路由已经切走」两种拒绝都硬填 0,而它们说的是"这条不该发"不是
  "你没额度了",记下来会让节流器从此闭嘴;真正的耗尽由最后一条**成功**的发送带回 0。
