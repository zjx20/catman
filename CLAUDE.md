...
- **额度花光了不是绝路,`/nop` 就是那条出路**:用户随便发一句话都带来新的
  ...
  那句提示就恰好在最需要它的时候消失。
  口令这一侧要成立,网关收到 `/nop` 时必须把节流器重新开闸
  (`TurnContext.resetProgress`);少了它,节流器还记着"该等满最后一档",
  表现就是"照做了却没反应",而那句提示恰恰是我们让用户信的。所以 `/nop` 归**人格**
  执行,不归信使:信使就地消化掉的话,节流器压根不知道额度回来了。
  开闸之外它**一个字都不回**。曾经还要求"真的发一条回话出去",理由是网关手里的余量
  靠发送响应带回来、不发就停在 0 —— 那份缓存在下一条的第 ③ 步里已经删了,理由跟着
  失效,只剩代价:确认话术要从刚续回来的额度里花掉一条,挤掉的正是那条进度。
  积压排空也不靠它,`courier/core.ts` 的 `accept()` 每收一条来信就 `kick()` 一次。
